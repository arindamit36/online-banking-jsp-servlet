...
<h2 class="section-header">Update Profile<span>.</span></h2>
	...
	<%
				...
				ResultSet rs=st.executeQuery("SELECT * FROM bankuserinfo WHERE USERID='"+session.getAttribute(Contents.WP_USER)+"'");
				if(rs.next()){
			%>
			<%
	try{
		...
		case Contents.WP_ST_SUCCESS:
			out.println("<center><font color=\"green\">Profile updated Successfully !</font></center>");
			break;
	...
	<tr>
		...
		<td width="350" style="padding:5px;">
			<input type="text" name="userid" placeholder="Account No" value="<%=rs.getString("USERID")%>" maxlength="12" required="" class="form-control" />
		</td>
	</tr>
	<tr>
...
<div class="row">
	<div class="col-md-6 col-sm-6 col-xs-6" >
		<button type="button" onclick="document.location='bank.jsp'" class="btn submit" style="width:200%; margin-bottom: 20px">back to main page</button>
	</div>
	</div>
	</div>
